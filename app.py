...
# Load a speaker embedding (example uses a random speaker embedding)
...
def generate_speech(message):
    ...
    print("Calling Generate Speech Function")
    print(message)
    audio_file_path = "D:\Ascentt_Ascentt\speed_testing\static\speech.wav"
    sf.write(audio_file_path, speech.numpy(), 16000)  # Save speech with a sample rate of 16kHz
...
#         # logging.debug(f"Places result for type {place_type}: {places_result}")
...
#             message = f"Please limit your speed to {speed_limit_in_words} km/h in front of {place}"
...
def get_nearby_institutions():
    try:
        ...
        places = []
        types = ["school", "hospital", "hindu_temple"]
